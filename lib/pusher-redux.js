...
module.exports.unsubscribeChannel = function (channelName) {
  addToQueue(function () {
    ...
    for (var eventName in channelSubs) {
      if (channelSubs.hasOwnProperty(eventName)) {
        for (var actionType in channelSubs[eventName]) {
          if (channelSubs[eventName].hasOwnProperty(actionType)) {
            channel.unbind(eventName, channelSubs[eventName][actionType]);
            delete channelSubs[eventName][actionType];
          }
        }
      }
    }
    ...
    delete config.subscriptions[channelName];
  });
};
